...
WITH ranked_data AS (
    ...
    FROM
        `mpi-main.facebook_raw.ads_insights`
),
...
ad_data AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(data,'$.id') ORDER BY _sdc_extracted_at DESC) AS row_num
    FROM `mpi-main.facebook_raw.ads`
),
...
adset_data AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(data,'$.id') ORDER BY _sdc_extracted_at DESC) AS row_num
    from `mpi-main.facebook_raw.ad_sets`
),
...
campaign_data AS (
    ...
    FROM `mpi-main.facebook_raw.campaigns`
),
