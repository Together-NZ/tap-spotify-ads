...
WITH parsed_data AS (
    ...
    FROM
        `mpi-main.dv360_raw.dv360_youtube` where LOWER(JSON_VALUE(data, "$.Insertion Order")) LIKE '%mpi%'
)
...
FROM
    parsed_data
WHERE
    lower(campaign_name) like '%mpi%' and lower(campaign_name) like '%yt%' and row_num = 1