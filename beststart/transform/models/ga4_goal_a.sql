...
filtered_creatives AS (
  SELECT
    * EXCEPT(sessionManualAdContent),
    CASE
      WHEN sessionManualAdContent IS NULL THEN NULL
      WHEN STRPOS(sessionManualAdContent, '_') = 0 THEN sessionManualAdContent
      ELSE REGEXP_REPLACE(sessionManualAdContent, r'^.*_', '')
