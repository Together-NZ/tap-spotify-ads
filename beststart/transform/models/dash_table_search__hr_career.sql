...
with dash_table AS (
SELECT * FROM `best-start-main.google_ads_search_transformed__hr_career.google_ads_search__hr_career`
)
SELECT *,
CASE WHEN 
...
NULL AS ad_format,
CASE WHEN 'INTENT' IN (select distinct funnel from `best-start-main.dash_table.dash_table`) then 'INTENT'
