...
WITH parsed_data AS (
    ...
    FROM
        `wendys-main.dv360_raw.dv360_youtube`
)
...
SELECT
    ...
    CASE WHEN
        ARRAY_LENGTH(SPLIT(youtube_ad,'_')) >=8 THEN SPLIT(youtube_ad,'_')[OFFSET(5)]
        ELSE 'Other'
    END AS ad_format_detail,
    CASE WHEN
        ARRAY_LENGTH(SPLIT(youtube_ad,'_')) >=8 THEN SPLIT(youtube_ad,'_')[OFFSET(7)]
        ELSE 'Other'
    END AS creative_descr,
    'No Valid' as ad_format,
    CASE WHEN 
        ARRAY_LENGTH(SPLIT(campaign_name,'_')) <=1 THEN 'Other'
