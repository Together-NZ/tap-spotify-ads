...
FROM `wendys-main.tiktok_raw.ads` ),
ad_campaign AS (
...
ad_data AS (
  ...
  FROM `wendys-main.tiktok_raw.ads_basic_data_metrics_by_day`
),
deduplicate_ad as (
...
ad_video AS (
  SELECT 
    ...
    JSON_VALUE(data, '$.stat_time_day') AS date,
    ROW_NUMBER() OVER (PARTITION BY SAFE_CAST(JSON_VALUE(data, '$.ad_id') AS INT64),JSON_VALUE(data, '$.stat_time_day')) as row_num
  FROM `wendys-main.tiktok_raw.ads_video_play_metrics_by_day`
),
...
ad_name as creative_name,
    ...
    CASE WHEN ARRAY_LENGTH(SPLIT(campaign_name,'_'))>=2 THEN SPLIT(campaign_name,'_')[OFFSET(1)] ELSE campaign_name END AS campaign_descr
FROM 
    final
