...
with dash_table AS (
SELECT * FROM `liquorland-main.google_ads_search_transformed.google_ads_search`
)
SELECT *,
...
CASE WHEN 'INTENT' IN (select distinct funnel from `liquorland-main.dash_table.dash_table`) and lower(publisher) != 'demand gen' then 'INTENT'
