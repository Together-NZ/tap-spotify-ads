...
WITH cm360reference AS (
    ...
    WHERE 
        LOWER(JSON_VALUE(JSON_EXTRACT(data, "$.advertiser"))) = 'liquorland'
        -- filter the site name from the cm360 dataset
        AND LOWER(JSON_VALUE(JSON_EXTRACT(data, "$.site"))) NOT IN ('the trade desk', 'ttd', 'facebook','meta','dv360','twitch', 'programmatic', 'dart', 'google ads', 'sem')
