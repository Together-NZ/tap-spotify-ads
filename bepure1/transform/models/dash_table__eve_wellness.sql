...
WITH dash_table AS (
    ...
    FROM `bepure-main.facebook_transformed__eve_wellness.facebook__eve_wellness`
    UNION ALL
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion
           ...
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `bepure-main.tiktok_transformed__eve_wellness.tiktok__eve_wellness`
    UNION ALL
    ...
    FROM `bepure-main.google_ads_search_transformed__eve_wellness.google_ads_demand__eve_wellness`
),
with_channel as (
...
where lower(campaign_name) like '%eve%'),
campaign_base AS (
       SELECT *,
