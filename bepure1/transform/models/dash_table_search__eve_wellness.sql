...
with dash_table AS (
SELECT * FROM `bepure-main.google_ads_search_transformed__eve_wellness.google_ads_search__eve_wellness`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `bepure-main.dash_table__eve_wellness.dash_table__eve_wellness`) then 'INTENT'
