...
with dash_table AS (
SELECT * FROM `bepure-main.google_ads_search_transformed__bepure.google_ads_search__bepure`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `bepure-main.dash_table__bepure.dash_table__bepure`) then 'INTENT'
