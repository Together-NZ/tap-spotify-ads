...
SELECT 
  ...
  FROM `arvida-main.hivestack_raw.report`),
  deduplicate_data AS (
...
SELECT SUM(impressions) as impressions,SUM(media_cost) as media_cost,creative_name, ad_format,campaign_name, 0 as clicks ,audience_name,ad_format_detail,DATE(date) as date ,publisher, campaign_descr,creative_descr FROM populate_data group by creative_name, campaign_name,ad_format_detail,campaign_descr,audience_name,publisher,creative_descr,date,ad_format