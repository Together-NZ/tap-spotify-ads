...
with dash_table AS (
SELECT * FROM `arvida-main.google_ads_search_transformed.google_ads_search`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `arvida-main.dash_table.dash_table`) then 'INTENT'
