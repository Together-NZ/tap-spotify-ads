...
SAFE_CAST(campaign_start_date AS STRING) AS campaignf_start_date,
...
SAFE_CAST(creative_descr AS STRING) AS creative_descr FROM `public-trust-main.dash_table.dash_table` ) UNION ALL (SELECT 
EXTRACT(MONTH FROM date) AS segments_month,
EXTRACT(QUARTER FROM date) AS segments_quarter,
EXTRACT(WEEK FROM date) AS segments_week,
EXTRACT(YEAR FROM date) AS segments_year,
