...
distinct_campaign AS (
    ...
    video_metrics AS (
        SELECT * FROM `together-internal.google_ads_data_transfer.ads_VideoNonClickStats_6544860891`
        where customer_id=1287289620
    ),
    video_view AS (
        SELECT metrics_video_views as video_views, metrics_video_quartile_p75_rate as video_p75,
        metrics_video_quartile_p25_rate as video_P25,
        metrics_video_quartile_p50_rate as video_p50,
        metrics_video_quartile_p100_rate as video_p100,
        campaign_id,
        ad_group_ad_ad_id,
        _DATA_DATE ,
        ROW_NUMBER () OVER (PARTITION BY campaign_id, ad_group_ad_ad_id,_DATA_DATE,video_p100,video_p50,video_p25,video_p75 ORDER BY _DATA_DATE) as row_num
        FROM video_metrics
    ),
    deduplicated_video AS (
        SELECT * FROM video_view where row_num=1
    ),
    result_data AS (
SELECT  
    ...
    vd.video_p75 * SUM(ad_stat.metrics_impressions) as video_75_completion,
    vd.video_P25 * SUM(ad_stat.metrics_impressions) as video_25_completion,
    vd.video_p50 * SUM(ad_stat.metrics_impressions) as video_50_completion,
    vd.video_p100 * SUM(ad_stat.metrics_impressions) as video_completion,
...
LEFT JOIN
    deduplicated_video AS vd
ON 
    vd.ad_group_ad_ad_id = ad_stat.ad_group_ad_ad_id
LEFT JOIN
    distinct_ad_group AS ad_group
ON 
    ad_group.ad_group_id = ad_stat.ad_group_id
where ad_stat.customer_id = 1287289620 and (lower(publisher) not like '%search%' and lower(publisher) not like '%performance max%')
GROUP BY 
