...
with dash_table AS (
SELECT * FROM `bepure-main.google_ads_search_transformed__eve_wellness_au.google_ads_search__eve_wellness_au`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `bepure-main.dash_table__eve_wellness_au.dash_table__eve_wellness_au`) then 'INTENT'
