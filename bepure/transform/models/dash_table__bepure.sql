...
WITH dash_table AS (
    ...
    FROM `bepure-main.dv360_transformed.dv360_standard`
    
    UNION ALL
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion, video_25_completion as video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `bepure-main.dv360_transformed.dv360_youtube`
...
from `bepure-main.cm360_transformed.cm360_direct_buy`
    UNION ALL
    ...
    FROM `bepure-main.google_ads_search_transformed__bepure.google_ads_demand__bepure`
),
with_channel as (
...
where lower(campaign_name) like '%bep%'
),
campaign_base AS (
       SELECT *,
