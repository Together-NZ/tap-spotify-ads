...
WITH ranked_data AS (
    ...
    FROM
        `inghams-main.facebook_raw__waitoa.ads_insights`
),
...
ad_data AS (
    ...
    FROM `inghams-main.facebook_raw__waitoa.ads`
),
...
adset_data AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(data,'$.id') ORDER BY _sdc_extracted_at) AS row_num
    from `inghams-main.facebook_raw__waitoa.ad_sets`
),
...
campaign_data AS (
    ...
    FROM `inghams-main.facebook_raw__waitoa.campaigns`
),
...
device_data AS (
    ...
    FROM 
        `inghams-main.facebook_raw__waitoa.ads_insights_delivery_device`
),
