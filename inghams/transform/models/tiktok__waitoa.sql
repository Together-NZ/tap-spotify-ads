...
FROM `inghams-main.tiktok_raw__waitoa.ads` ),
ad_data AS (
  SELECT 
  ...
  FROM `inghams-main.tiktok_raw__waitoa.ads_basic_data_metrics_by_day`
),
deduplicate_ad as (
...
ad_video AS (
  SELECT 
    ...
    JSON_VALUE(data, '$.stat_time_day') AS date,
    ROW_NUMBER() OVER (PARTITION BY SAFE_CAST(JSON_VALUE(data, '$.ad_id') AS INT64),JSON_VALUE(data, '$.stat_time_day')) as row_num
  FROM `inghams-main.tiktok_raw__waitoa.ads_video_play_metrics_by_day`
),
