...
WITH parsed_data AS (
    ...
    FROM
        `ballance-main.ttd_raw.standard_streams`
),
ranked_data AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
            PARTITION BY
                Date, partner_id, advertiser_id, campaign_id, ad_group_id, ad_format, 
                advertiser, creative_id, deal_id, ad_server_creative_placement_id
            ORDER BY
