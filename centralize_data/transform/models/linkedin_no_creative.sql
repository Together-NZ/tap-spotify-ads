...
WITH daily_stats AS (SELECT
  SAFE_CAST(JSON_VALUE(data, '$.videoCompletions') AS INT64) AS video_completion,
  SAFE_CAST(JSON_VALUE(data, '$.comments') AS INT64) AS comments,
  JSON_VALUE(data, '$.costInUsd') AS cost_in_usd,
  SAFE_CAST(JSON_VALUE(data, '$.costInLocalCurrency') AS FLOAT64) AS media_cost,
  SAFE_CAST(JSON_VALUE(data, '$.impressions') AS INT64) AS impressions,
  SAFE_CAST(JSON_VALUE(data, '$.clicks') AS INT64) AS clicks,
  SAFE_CAST(JSON_VALUE(data, '$.totalEngagements') AS INT64) AS totalEngagements,
  SAFE_CAST(JSON_VALUE(data, '$.fullScreenPlays') AS INT64) AS fullScreenPlays,
  SAFE_CAST(JSON_VALUE(data, '$.videoStarts') AS INT64) AS video_starts,
  SAFE_CAST(JSON_VALUE(data, '$.videoViews') AS INT64) AS video_views,
  SAFE_CAST(JSON_VALUE(data, '$.videoFirstQuartileCompletions') AS INT64) AS video_25_completion,
  SAFE_CAST(JSON_VALUE(data, '$.videoMidpointCompletions') AS INT64) AS video_50_completion,
  SAFE_CAST(JSON_VALUE(data, '$.videoThirdQuartileCompletions') AS INT64) AS video_75_completion,
  SAFE_CAST(JSON_VALUE(data, '$.likes') AS INT64) AS likes,
  SAFE_CAST(JSON_VALUE(data, '$.follows') AS INT64) AS follows,
  SAFE_CAST(JSON_VALUE(data, '$.commentLikes') AS INT64) AS commentLikes,
  SAFE_CAST(JSON_VALUE(data, '$.landingPageClicks') AS INT64) AS landingPageClicks,
  DATE(
  TIMESTAMP(JSON_VALUE(data, '$.day')),
  "Pacific/Auckland"
  ) AS date,
...
FROM
  `together-internal.linkedin_raw.ad_analytics_by_creative`),
...
campaign_stats AS (      SELECT
    ...
    JSON_VALUE(data, '$.objectiveType') as campaign_type,
    ...
    from `together-internal.linkedin_raw.campaigns`),
