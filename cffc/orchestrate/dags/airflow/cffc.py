...
with models.DAG(
    ...
    list = ["cffc"]
    for label in list:
        real_label='retirement_commission'
        kube_hivestack = KubernetesPodOperator(
            name=f"cffc-hivestack-to-bigquery-{label}",
            task_id=f"cffc-hivestack_to_bigquery_{label}",
            namespace="composer-user-workloads",
            image=IMAGE,
            arguments=["--environment=prod", "invoke",f"dbt-bigquery:hivestack_{label}_models"],
            container_resources=k8s_models.V1ResourceRequirements(
                limits={"memory": "1000M", "cpu": "500m"},
            ),
            env_vars = set_env_vars_hivestack(label),
        )
        label_kube_list.setdefault(real_label, []).append(kube_hivestack)
        kube_linkedin = KubernetesPodOperator(
            name=f"cffc-linkedin-to-bigquery-{label}",
            task_id=f"cffc-linkedin_to_bigquery_{label}",
            namespace="composer-user-workloads",
            image=IMAGE,
            arguments=["--environment=prod", "run","tap-linkedin-ads","target-bigquery",f"dbt-bigquery:linkedin_{label}_models"],
            container_resources=k8s_models.V1ResourceRequirements(
                limits={"memory": "1000M", "cpu": "500m"},
            ),
            env_vars = set_env_vars_linkedin(label),
        )
        label_kube_list.setdefault(real_label, []).append(kube_linkedin)
        ...
        label_kube_list.setdefault(real_label, []).append(kube_cm360)
        ...
        label_kube_list.setdefault(real_label, []).append(kube_snapchat)
        ...
        label_kube_list.setdefault(real_label, []).append(kube_facebook)
        ...
        label_kube_list.setdefault(real_label, []).append(kube_dv360)
        ...
        label_kube_list.setdefault(real_label, []).append(kube_ttd)
        kube_cm360 >> kube_ttd
    ...
    for label in search_list: 
        kube_google_ads_search = KubernetesPodOperator(
            ...
            namespace="composer-user-workloads",
            image=IMAGE,
            arguments=["--environment=prod", "invoke","dbt-bigquery","run","--select",f"google_ads_{label}_models"],
            container_resources=k8s_models.V1ResourceRequirements(
                limits={"memory": "1000M", "cpu": "500m"},
            ),
        ...
        google_ads_search_list.setdefault(label, []).append(kube_google_ads_search)
        dash_search_list.setdefault(label, []).append(kube_dash_search)
    overall_list = ['retirement_commission','sorted','sorted_in_school']
    for list in overall_list:
        real_name = list
        kube_dash = KubernetesPodOperator(
            name=f"cffc-dash-to-bigquery-{real_name}",
            task_id=f"cffc-dash_to_bigquery-{real_name}",
            namespace="composer-user-workloads",
            image=IMAGE,
            arguments=["--environment=prod", "invoke","dbt-bigquery","run","--select",f"dash_table__{real_name}"],
            container_resources=k8s_models.V1ResourceRequirements(
                limits={"memory": "1000M", "cpu": "500m"},
            ),
        ...
        if real_name=='retirement_commission':
            for upstream in label_kube_list[real_name]:
                for upstream2 in label_kube_ga4_list[real_name]:
                    upstream >> kube_dash >> kube_dash_union >> upstream2
                
        else:
            for upstream in google_ads_search_list[real_name]:
                for upstream2 in dash_search_list[real_name]:
                    for upstream3 in label_kube_ga4_list[real_name]:
                        upstream >> upstream2 >> kube_dash >> kube_dash_union >> upstream3
