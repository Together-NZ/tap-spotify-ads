...
WITH channels as (
  SELECT *,
    ...
    CASE 
        ...
        ELSE 'Unknown'
    END AS channel
  FROM {{ ref('ga4_goal_a__retirement_commission') }}
),with_channel AS (
select *,
  CASE 
...
funnel_campaign AS (
     select distinct funnel, campaign_name from `cffc-main.dash_table__retirement_commission.dash_union__retirement_commission`
),
dash AS (
     SELECT DISTINCT 
     ...
     FROM `cffc-main.dash_table__retirement_commission.dash_union__retirement_commission`
),
deduplicate_dash AS (
