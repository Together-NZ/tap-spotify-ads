...
WITH channels as (
  SELECT *,
    ...
    CASE 
        ...
        ELSE 'Unknown'
    END AS channel
  FROM {{ ref('ga4_goal_a__smart_sorted') }}
),with_channel AS (
select *,
  CASE 
...
funnel_campaign AS (
     select distinct funnel, campaign_name from `cffc-main.dash_table__smart_sorted.dash_union__smart_sorted`
),
dash AS (
     SELECT DISTINCT 
     ...
     FROM `cffc-main.dash_table__smart_sorted.dash_union__smart_sorted`
),
deduplicate_dash AS (
