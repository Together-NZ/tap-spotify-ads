...
WITH channels as (
  SELECT *,
    ...
    CASE 
        ...
        ELSE 'Unknown'
    END AS channel
  FROM {{ ref('ga4_goal_a__sorted_in_school') }}
),with_channel AS (
select *,
  CASE 
...
funnel_campaign AS (
     select distinct funnel, campaign_name from `cffc-main.dash_table__sorted_in_school.dash_union__sorted_in_school`
),
dash AS (
     SELECT DISTINCT 
     ...
     FROM `cffc-main.dash_table__sorted_in_school.dash_union__sorted_in_school`
),
deduplicate_dash AS (
