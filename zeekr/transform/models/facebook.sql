...
WITH ranked_data AS (
    ...
    FROM
        `zeekr-main.facebook_raw.ads_insights_action_video_type`
),
...
ad_data AS (
    ...
    FROM `zeekr-main.facebook_raw.ads`
),
...
adset_data AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(data,'$.id') ORDER BY _sdc_extracted_at) AS row_num
    from `zeekr-main.facebook_raw.ad_sets`
),
...
campaign_data AS (
    ...
    FROM `zeekr-main.facebook_raw.campaigns`
),
...
device_data AS (
    ...
    FROM 
        `zeekr-main.facebook_raw.ads_insights_delivery_device`
),
