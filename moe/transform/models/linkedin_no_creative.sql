...
campaign_stats AS (      SELECT
    ...
    from `moe-main.linkedin_raw.campaigns`),
deduplicated_campaign_data AS (SELECT * EXCEPT(row_num) FROM campaign_stats where row_num = 1)
SELECT stat_creative.* EXCEPT(media_cost), campaign_data.* EXCEPT(campaign_id) ,
CASE WHEN creative_id='627575416' and date='2025-07-01' THEN 0 else media_cost END AS media_cost
FROM 
