...
WITH dedupllicate_data AS (
    ...
    FROM
        `moe-main.dv360_raw.dv360_standard`
)
SELECT * ,
    CASE 
        WHEN ARRAY_LENGTH(SPLIT(campaign_name, '_')) > 3 THEN
            CASE 
    ...
    CASE 
        ...
        ELSE 'Dv360'
    END AS publisher,
    CASE WHEN ARRAY_LENGTH(SPLIT(line_item,'_')) >= 8 THEN SPLIT(line_item,'_')[OFFSET(7)] 
    ELSE 'Other' END AS audience_name,
    --SPLIT(line_item, '_')[OFFSET(7)] AS audience_name,
