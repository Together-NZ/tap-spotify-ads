...
with dash_table AS (
SELECT * FROM `moe-main.google_ads_search_transformed.google_ads_search`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `moe-main.dash_table.dash_table`) then 'INTENT'
