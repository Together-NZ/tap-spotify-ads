...
WITH ranked_data AS (
    ...
    FROM
        `doconservation-main.facebook_raw.ads_insights`
),
...
ad_data AS (
    ...
    FROM `doconservation-main.facebook_raw.ads`
),
...
adset_data AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(data,'$.id') ORDER BY _sdc_extracted_at) AS row_num
    from `doconservation-main.facebook_raw.ad_sets`
),
...
campaign_data AS (
    ...
    FROM `doconservation-main.facebook_raw.campaigns`
),
