...
WITH deduplicated_data AS (
  SELECT 
    ...
    JSON_VALUE(data,'$.sessionDefaultChannelGroup') as sessionDefaultChannel,
    JSON_VALUE(data,'$.customEvent:form_type') as customEvent_form_type,
    JSON_VALUE(data,'$.customEvent:event_category') as customEvent_event_category,
    ...
    CASE 
        ...
        ELSE lower(SPLIT(JSON_VALUE(data, '$.sessionSourceMedium'), '/')[SAFE_OFFSET(0)])
    END AS site_name,
  ...
  FROM 
    `doconservation-main`.`ga4_raw`.`goal`

),
custom_eventname AS (
  select * except(eventName),
  case when lower(eventName) = 'form_load' and lower(customEvent_form_type)like"keep me updated" then 'keep_me_updated_start'
  when lower(eventName) = 'page_view' and lower(customEvent_event_category)like "%dealer locator%"  then 'dealer_locator'
  when lower(eventName) = 'form_load' and lower(customEvent_form_type)='test drive booking' then 'book_a_test_drive'
  when lower(eventName) = 'form_load' and lower(customEvent_form_type)='request a quote' then 'offer_request_start'
  else eventName
  end as eventName
...
filtered_creatives as (
  ...
  CASE WHEN LOWER(sessionManualAdContent) like '%vol0%' THEN SPLIT(sessionManualAdContent,'_')[OFFSET(ARRAY_LENGTH(SPLIT(sessionManualAdContent,'_'))-1)]
  else sessionManualAdContent
  end as sessionManualAdContent
  from custom_eventname
)

-- Select only the latest row for each unique combination of keys
SELECT
  date,
  customEvent_form_type,
  customEvent_event_category,
  sessionSourceMedium,
