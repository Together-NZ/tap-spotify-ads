  ...
  SELECT 
    ...
    ROW_NUMBER() OVER (
      PARTITION BY 
        ...
        CAST(SAFE_CAST(JSON_VALUE(data, '$.eventValue') AS FLOAT64) AS STRING),
        CAST(SAFE_CAST(JSON_VALUE(data, '$.eventCount') AS FLOAT64) AS STRING)
      ORDER BY _sdc_extracted_at DESC
    ) AS row_num

  FROM 
    `barfoot-and-thompson-main`.`ga4_raw`.`goal`

),

filtered_creatives as (
  SELECT * except(sessionManualAdContent),
  CASE WHEN LOWER(sessionManualAdContent) like '%bnt%' THEN SPLIT(sessionManualAdContent,'_')[OFFSET(ARRAY_LENGTH(SPLIT(sessionManualAdContent,'_'))-1)]
