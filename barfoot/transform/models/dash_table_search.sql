...
with dash_table AS (
SELECT * FROM `barfoot-and-thompson-main.google_ads_search_transformed.google_ads_search`
)
SELECT *,
CASE WHEN 
...
NULL AS ad_format, 
CASE WHEN 'INTENT' IN (select distinct funnel from `barfoot-and-thompson-main.dash_table.dash_table`) then 'INTENT'
