...
WITH deduplicated_data AS (
  SELECT 
    ...
    CASE 
        ...
        WHEN (LOWER(JSON_VALUE(data, '$.sessionSourceMedium')) LIKE '%meta%' 
  ...
  FROM 
    `geely-main`.`ga4_raw__geely`.`goal`

),

filtered_creatives as (
  SELECT * except(sessionManualAdContent),
  CASE WHEN LOWER(sessionManualAdContent) like '%gel%' THEN SPLIT(sessionManualAdContent,'_')[OFFSET(ARRAY_LENGTH(SPLIT(sessionManualAdContent,'_'))-1)]
