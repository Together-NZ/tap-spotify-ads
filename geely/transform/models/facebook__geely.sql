...
WITH ranked_data AS (
    ...
    FROM
        `geely-main.facebook_raw__geely.ads_insights`
),
...
ad_data AS (
    ...
    FROM `geely-main.facebook_raw__geely.ads`
),
...
adset_data AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(data,'$.id') ORDER BY _sdc_extracted_at) AS row_num
    from `geely-main.facebook_raw__geely.ad_sets`
),
...
campaign_data AS (
    ...
    FROM `geely-main.facebook_raw__geely.campaigns`
),
