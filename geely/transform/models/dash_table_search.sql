...
with dash_table AS (
SELECT * FROM `geely-main.google_ads_search_transformed.google_ads_search`
)
SELECT *,
...
CASE WHEN 'INTENT' IN (select distinct funnel from `geely-main.dash_table.dash_table`) and lower(publisher) != 'demand gen' then 'INTENT'
