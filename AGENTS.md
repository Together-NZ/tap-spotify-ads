# AGENTS.md - AI Agent Development Guide for tap-spotifyads
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from SpotifyAds.

### Key Components

1. **Tap Class** (`tap_spotifyads/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_spotifyads/client.py`): Handles API communication and authentication
1. **Streams** (`tap_spotifyads/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_spotifyads/streams.py`
...
```python
class MyNewStream(SpotifyAdsStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(SpotifyAdsStream):
...
```bash
tap-spotifyads --config config.json --discover
tap-spotifyads --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_spotifyads/tap.py`
...
```bash
# .env.example
TAP_SPOTIFYADS_API_URL=https://api.example.com
TAP_SPOTIFYADS_API_KEY=your_api_key_here
TAP_SPOTIFYADS_BATCH_SIZE=100  # New setting
```
