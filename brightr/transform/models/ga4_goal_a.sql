...
WITH deduplicated_data AS (
  ...
  FROM 
    `brightr-main`.`ga4_raw`.`goal`

),

filtered_creatives as (
  SELECT * except(sessionManualAdContent),
  CASE WHEN LOWER(sessionManualAdContent) like '%bri%' THEN SPLIT(sessionManualAdContent,'_')[OFFSET(ARRAY_LENGTH(SPLIT(sessionManualAdContent,'_'))-1)]
