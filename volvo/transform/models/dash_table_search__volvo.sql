...
with dash_table AS (
SELECT * FROM `volvo-main.google_ads_search_transformed__volvo.google_ads_search__volvo`
)
SELECT *,
...
CASE WHEN 'INTENT' IN (select distinct funnel from `volvo-main.dash_table__volvo.dash_table__volvo`) and lower(publisher) != 'demand gen' then 'INTENT'
