...
with dash_table AS (
SELECT * FROM `ballance-main.google_ads_search_transformed.google_ads_search`
)
SELECT *
EXCEPT(row_num),
CASE WHEN 
...
null as video_views,
CASE WHEN 'INTENT' IN (select distinct funnel from `ballance-main.dash_table.dash_table`) then 'INTENT'
