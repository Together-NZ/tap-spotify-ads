...
WITH ranked_data AS (
    ...
    FROM
        `real-nz-main.facebook_raw__mountain.ads_insights_action_video_type`
),
...
ad_data AS (
    ...
    FROM `real-nz-main.facebook_raw__mountain.ads`
),
...
adset_data AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY JSON_VALUE(data,'$.id') ORDER BY _sdc_extracted_at) AS row_num
    from `real-nz-main.facebook_raw__mountain.ad_sets`
),
...
campaign_data AS (
    ...
    FROM `real-nz-main.facebook_raw__mountain.campaigns`
),
...
device_data AS (
    ...
    FROM 
        `real-nz-main.facebook_raw__mountain.ads_insights_delivery_device`
),
