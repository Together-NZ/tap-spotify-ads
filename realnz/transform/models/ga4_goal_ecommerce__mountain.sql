...
WITH ecommerce AS (
    ...
    CASE 
        ...
        ELSE 'other'
    END AS site_name
    FROM `real-nz-main.ga4_raw__mountain.ecommerce_goal`
),
...
funnel_campaign AS (
     select distinct funnel, campaign_name from `real-nz-main.dash_table__mountain.dash_union__mountain`
),
final_result AS (
SELECT * ,
...
filtered_creatives as (
  ...
  CASE WHEN LOWER(sessionManualAdContent) like '%mtn%' THEN SPLIT(sessionManualAdContent,'_')[OFFSET(ARRAY_LENGTH(SPLIT(sessionManualAdContent,'_'))-1)]
