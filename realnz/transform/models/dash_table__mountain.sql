...
WITH dash_table AS (
    ...
    FROM `real-nz-main.facebook_transformed__mountain.facebook__mountain`
    UNION ALL
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion,video_25_completion AS video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `real-nz-main.dv360_transformed__mountain.dv360_standard__mountain`
    UNION ALL
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion,video_25_completion AS video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `real-nz-main.dv360_transformed__mountain.dv360_youtube__mountain`
    UNION ALL
...
from `real-nz-main.cm360_transformed__mountain.cm360_direct_buy__mountain`
    ...
    FROM `real-nz-main.google_ads_search_transformed__mountain.google_ads_demand__mountain`
    UNION ALL
      SELECT media_cost, impressions, CAST(0 AS INT64) AS clicks, 
           ...
           date,
    FROM `real-nz-main.hivestack_transformed__mountain.hivestack__mountain`
),
with_channel AS (
