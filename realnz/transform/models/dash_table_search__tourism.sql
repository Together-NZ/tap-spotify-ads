...
with dash_table AS (
SELECT * FROM `real-nz-main.google_ads_search_transformed__tourism.google_ads_search__tourism`
)
SELECT *,
...
NULL AS ad_format, 
CASE WHEN 'BOOK' IN (select distinct funnel from `real-nz-main.dash_table__tourism.dash_table__tourism`) and lower(publisher) != 'demand gen' then 'BOOK'
