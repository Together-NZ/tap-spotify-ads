...
WITH dash_table AS (
    ...
    FROM `kiwirail-main.facebook_transformed__interislander.facebook__interislander`
    ...
    FROM `kiwirail-main.dv360_transformed__interislander.dv360_standard__interislander` WHERE LOWER(campaign_name) not like '%yt%'
    UNION ALL
    SELECT media_cost, impressions, clicks, creative_name,audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion, video_25_completion as video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date,NULL AS conversions
    FROM `kiwirail-main.dv360_transformed__interislander.dv360_youtube__interislander`
    UNION ALL 
    ...
    FROM `kiwirail-main.google_ads_search_transformed__interislander.google_ads_demand__interislander`
),
with_channel as (
