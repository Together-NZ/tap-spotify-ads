...
WHERE row_num = 1 and ( (lower(campaign_name) not like '%mobile%' and lower(campaign_name) not like '%broadband%') OR (lower(eventName) ='purchase' and lower(hostName) like '%journey.contact%'and  (LOWER(serviceType) NOT LIKE '%broadband%' 
  or lower(serviceType) not like '%gas%' or serviceType not like '%B%')))
)
SELECT * except(eventName),
CASE WHEN 
  lower(hostName) not like '%contactmobile%' and eventName = 'purchase'
   THEN 'purchase_energy/broadband'
   WHEN LOWER(hostName) like '%contactmobile%' and eventName = 'purchase' THEN 'purchase_mobile'
