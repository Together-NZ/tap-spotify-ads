...
with dash_table AS (
    ...
    FROM `contact-energy-main.dv360_transformed__broadband.dv360_standard__broadband`
    UNION all
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion, video_25_completion as video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `contact-energy-main.dv360_transformed__broadband.dv360_youtube__broadband`
    UNION ALL
    SELECT media_cost, impressions, CAST(0 AS INT64) AS clicks, 
           ...
           date,
    FROM `contact-energy-main.hivestack_transformed__broadband.hivestack__broadband`
    UNION ALL
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion,video_played AS video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `contact-energy-main.facebook_transformed__broadband.facebook__broadband`
    UNION ALL
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion
           ...
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `contact-energy-main.tiktok_transformed__broadband.tiktok__broadband`
    UNION ALL 
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion, video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `contact-energy-main.ttd_transformed__broadband.ttd_transformed__broadband`
    UNION ALL
    ...
    FROM `contact-energy-main.google_ads_search_transformed__broadband.google_ads_demand__broadband`
        UNION ALL
    select media_cost,impressions,clicks,creative_name, audience_name, ad_format, ad_format_detail, 
        SAFE_CAST(0 AS INT64) AS video_completion,
        SAFE_CAST(0 AS INT64) AS video_25_completion,
...
from `contact-energy-main.cm360_transformed__broadband.cm360_direct_buy__broadband`
...
NULL AS campaign_mamual_cpc_enchanced_cpc_enabled,
...
END as media_format,
CASE WHEN 
       lower(campaign_name) like '%awareness%' THEN 'WE'
       ELSE 'ME'
END AS funnel
