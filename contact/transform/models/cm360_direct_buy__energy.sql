...
WITH cm360reference AS (
    ...
    FROM 
        `together-internal.cm360_raw__contact.cm360_report_stream`
    WHERE 
        LOWER(JSON_VALUE(JSON_EXTRACT(data, "$.advertiser"))) like '%contact%'
...
FROM cm360reference
WHERE 
    row_num = 1  and (lower(campaign_name) not like '%mobile%' and lower(campaign_name) not like '%broadband%')
{% if is_incremental() %}
