...
WITH deduplicated_data AS (
  SELECT 
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY JSON_VALUE(data, '$.eventCount'),_sdc_extracted_at DESC
    ) AS row_num

  FROM 
    `contact-energy-main`.`ga4_raw`.`goal`

),
custom_eventName AS (
  SELECT * EXCEPT(eventName),
  CASE WHEN lower(eventName) ='purchase' and lower(hostName) like '%contactmobile%' THEN 'purchase_mobile'
  WHEN lower(eventName) ='purchase' and lower(hostName) like '%journey.contact%'and  (LOWER(serviceType) NOT LIKE '%broadband%' 
  or lower(serviceType) not like '%gas%' or serviceType not like '%B%')
   THEN 'purchase_energy'
   WHEN lower(eventName) ='purchase' and(lower(hostName) like '%journey.contact%') and (lower(serviceType) like '%broadband%' or serviceType like '%B%') AND 
  LOWER(serviceType) NOT LIKE '%bottled%' THEN 'purchase_broadband'

  ELSE eventName END AS eventName
