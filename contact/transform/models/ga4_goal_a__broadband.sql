...
WHERE row_num = 1 and (lower(campaign_name) like '%broadband%' OR  (lower(eventName) ='purchase' and(lower(hostName) like '%journey.contact%') and (lower(serviceType) like '%broadband%' or serviceType like '%B%') AND 
  LOWER(serviceType) NOT LIKE '%bottled%' ))
)
