...
FROM `contact-energy-main.tiktok_raw.ads` ),
ad_data AS (
  SELECT 
  ...
  FROM `contact-energy-main.tiktok_raw.ads_basic_data_metrics_by_day`
),
deduplicate_ad as (
...
ad_video AS (
  SELECT 
    ...
    JSON_VALUE(data, '$.stat_time_day') AS date,
    ROW_NUMBER() OVER (PARTITION BY SAFE_CAST(JSON_VALUE(data, '$.ad_id') AS INT64),JSON_VALUE(data, '$.stat_time_day')) as row_num
  FROM `contact-energy-main.tiktok_raw.ads_video_play_metrics_by_day`
),
...
ad_name as creative_name,
    ...
    CASE 
        ...
        THEN 'Social Display'
        ELSE 'OTHER'   END AS media_format,
