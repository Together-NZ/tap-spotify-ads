...
WITH ecommerce AS (
    ...
    END AS site_name
    FROM `contact-energy-main.ga4_raw.ecommerce_goal`
),
deduplicated_data AS (
    select *,
    
    ROW_NUMBER() OVER (PARTITION BY campaign_name,product_name,sessionSourceMedium,date,sessionManualAdContent,site_name,itemVariant,hostName ORDER BY _sdc_extracted_at DESC) as row_num
    from ecommerce 
),
final_result AS (
SELECT * ,
