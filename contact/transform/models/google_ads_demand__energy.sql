{{ config(
 ...
 WITH ads AS (
        ...
        FROM 
             `contact-energy-main.google_ads_search_raw.ads_Ad_4362586476`
        --WHERE customer_id = 4308178898
...
campaign_data AS (
    ...
    FROM 
         `contact-energy-main.google_ads_search_raw.ads_Campaign_4362586476`
    --WHERE customer_id = 4308178898
),
distinct_campaign AS (
    ...
    ad_group AS (
        ...
        FROM 
             `contact-energy-main.google_ads_search_raw.ads_AdGroup_4362586476`
        --WHERE customer_id = 4308178898 
...
FROM 
    `contact-energy-main.google_ads_search_raw.ads_AdBasicStats_4362586476`AS ad_stat
LEFT JOIN 
    distinct_ads AS ad 
ON 
...
--where ad_stat.customer_id = 4308178898
WHERE (lower(publisher) not like '%search%' and lower(publisher) not like '%performance max%')
GROUP BY 
...
FROM result_data WHERE row_num = 1 and  (lower(campaign_name) not like '%broadband%' and lower(campaign_name) not like '%mobile%') 
