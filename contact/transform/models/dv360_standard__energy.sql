...
WITH dedupllicate_data AS (
    ...
    FROM
        `contact-energy-main.dv360_raw.dv360_standard`
),final as (
SELECT * ,
    CASE 
...
WHERE row_num = 1 and lower(campaign_name) like '%con%' and (lower(campaign_name) not like '%broadband%' and lower(campaign_name)not like '%mobile%')
)
SELECT f.* 
