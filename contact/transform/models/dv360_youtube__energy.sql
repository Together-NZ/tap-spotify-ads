...
WITH parsed_data AS (
    ...
    FROM
        `contact-energy-main.dv360_raw.dv360_youtube` where LOWER(JSON_VALUE(data, "$.Insertion Order")) LIKE '%con%'
)
...
FROM
    parsed_data
WHERE row_num = 1 and (lower(campaign_name) like '%energy%' or lower(campaign_name) like '%power%' or lower(campaign_name) like '%electricity%'
or lower(campaign_name) like '%brand%' or lower(campaign_name) like '%good plans%') and lower(campaign_name) like '%con%'
