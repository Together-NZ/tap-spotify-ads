...
WITH cm360reference AS (
    ...
    WHERE 
        JSON_VALUE(JSON_EXTRACT(data, "$.advertiser")) = 'Protect Your Breath'
...
CASE WHEN ARRAY_LENGTH(SPLIT(creative_name,'_')) >=8 then SPLIT(creative_name, '_')[SAFE_OFFSET(6)] else 'Other' END AS ad_format,
CASE WHEN ARRAY_LENGTH(SPLIT(creative_name,'_')) >=8 then SPLIT(creative_name, '_')[SAFE_OFFSET(7)] else 'Other' END AS creative_descr,
