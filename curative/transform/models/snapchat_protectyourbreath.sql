...
WITH ad_stat_filtered_duplicate AS (
    ...
    FROM (
        ...
        FROM `curative-main.snapchat_protectyourbreath_raw.ad_stats_daily`
    )
...
ad_details AS (
    ...
    FROM `curative-main.snapchat_protectyourbreath_raw.ads`
),
...
ad_squad AS (
    ...
    FROM `curative-main.snapchat_protectyourbreath_raw.ad_squads`
),
...
campaign AS (
    ...
    FROM `curative-main.snapchat_protectyourbreath_raw.campaigns`
),
...
SELECT * EXCEPT(ad_name), ad_name AS creative_name,
    ...
    CASE 
        ...
        ELSE 'Other'
    END AS media_format,
    CASE WHEN ARRAY_LENGTH(SPLIT(ad_name,'_')) >=8 THEN 
    SPLIT(ad_name, '_')[OFFSET(5)] ELSE 'Other' END AS ad_format_detail,
    CASE WHEN ARRAY_LENGTH(SPLIT(ad_name,'_')) >=8 THEN 
    SPLIT(ad_name, '_')[OFFSET(6)] else 'Other' END AS ad_format,
    CASE WHEN ARRAY_LENGTH(SPLIT(ad_name,'_')) >=8 THEN 
    SPLIT(ad_name, '_')[OFFSET(7)] ELSE 'Other' END AS creative_descr,
    CASE 
