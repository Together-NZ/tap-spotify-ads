...
with dash_table AS (
SELECT * FROM `curative-main.google_ads_creativenz_search_transformed.google_ads_search__creativenz`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `curative-main.dash_table__creativenz.dash_table__creativenz`) then 'INTENT'
