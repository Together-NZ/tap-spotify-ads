...
WITH deduplicated_data AS (
  ...
  FROM 
    `curative-main`.`ga4_fasd_raw`.`goal`

),

filtered_creatives as (
  SELECT * except(sessionManualAdContent),
  CASE WHEN LOWER(sessionManualAdContent) like '%fas%' THEN SPLIT(sessionManualAdContent,'_')[OFFSET(ARRAY_LENGTH(SPLIT(sessionManualAdContent,'_'))-1)]
