...
WITH dash_table AS (
    ...
    FROM `curative-main.ttd_fasd_transformed.ttd_fasd_transformed`
    ...
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion, video_25_completion as video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `curative-main.dv360_fasd_transformed.dv360_fasd_standard`
    
    UNION ALL
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion, video_25_completion as video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `curative-main.dv360_fasd_transformed.dv360_fasd_youtube`
    UNION ALL
    ...
    SELECT media_cost, impressions, clicks, creative_name, audience_name, ad_format, ad_format_detail, video_completion,video_25_completion,video_50_completion,video_75_completion,video_played AS video_views,
           campaign_name, publisher, campaign_descr, creative_descr, date(date) as date
    FROM `curative-main.facebook_fasd_transformed.facebook_fasd`
...
from `curative-main.cm360_fasd_transformed.cm360_fasd_direct_buy`
UNION ALL 
    ...
    FROM `curative-main.google_ads_fasd_search_transformed.google_ads_demand__fasd`
    UNION ALL
    SELECT media_cost, impressions, CAST(0 AS INT64) AS clicks, 
           ...
           date,
    FROM `curative-main.hivestack_fasd_transformed.hivestack_fasd`
), with_channel AS (
...
where lower(campaign_name) like '%fas%'
),
campaign_base AS (
       SELECT *,
