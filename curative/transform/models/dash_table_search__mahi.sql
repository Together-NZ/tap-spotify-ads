...
with dash_table AS (
SELECT * FROM `curative-main.google_ads_mahi_search_transformed.google_ads_search__mahi`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `curative-main.dash_table__mahi.dash_table__mahi`) then 'INTENT'
