...
with dash_table AS (
SELECT * FROM `curative-main.google_ads_fasd_search_transformed.google_ads_search__fasd`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `curative-main.dash_table__fasd.dash_table__fasd`) then 'INTENT'
