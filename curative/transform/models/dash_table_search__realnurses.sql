...
with dash_table AS (
SELECT * FROM `curative-main.google_ads_realnurses_search_transformed.google_ads_search__realnurses`
)
SELECT *,
CASE WHEN 
...
CASE WHEN 'INTENT' IN (select distinct funnel from `curative-main.dash_table__realnurses.dash_table__realnurses`) then 'INTENT'
