...
with models.DAG(
    ...
    for id, label in ga4_list.items():
        ...
        ga4_label_task.setdefault(label, []).append(kube_ga4)
    ...
    for label in tiktok_labels:
        for tiktok in tiktok_task_list[label]:
            for dash in dash_list[label]:
                tiktok >> dash
    ga4_lists=['fasd','mahi','realnurses']
    for label in ga4_lists:
        for ga4 in ga4_label_task[label]:
...
with models.DAG(
    ...
    hivestack_list = {env["TAP_HIVESTACK_PROTECTYOURBREATH_ID"]:'protectyourbreath',env["TAP_HIVESTACK_FASD_ID"]:'fasd'}
    ...
    dv360_list = {env["DV360_FASD_ID"]:'fasd',env["DV360_MAHI_ID"]:'mahi',env["DV360_PROTECTYOURBREATH_ID"]:'protectyourbreath',
                  env["DV360_CREATIVENZ_ID"]:'creativenz'}
