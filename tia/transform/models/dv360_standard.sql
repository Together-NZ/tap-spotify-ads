...
WITH dedupllicate_data AS (
    ...
    FROM
        `tia-main.dv360_raw.dv360_standard`
    WHERE SAFE.PARSE_DATE('%Y/%m/%d', JSON_VALUE(data, "$.Date")) IS NOT NULL
)
SELECT * ,
    CASE 
        WHEN SPLIT(campaign_name, '_')[OFFSET(3)] LIKE '%YT%' AND ARRAY_LENGTH(SPLIT(campaign_name, '_')) >= 8 THEN 'Youtube Video'
        WHEN ARRAY_LENGTH(SPLIT(campaign_name, '_')) >= 8 THEN SPLIT(campaign_name, '_')[OFFSET(3)]
        ELSE 'Other'
    END AS media_format,
    CASE 
        ...
        ELSE 'Dv360'
    END AS publisher,
    CASE WHEN ARRAY_LENGTH(SPLIT(line_item, '_'))>=8 THEN SPLIT(line_item, '_')[OFFSET(7)] ELSE 'Other' END  AS audience_name,
